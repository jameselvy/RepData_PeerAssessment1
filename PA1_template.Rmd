...
```
The 5-minute interval that contains the maximum number of steps is located at ow number (index) `r which.max(activity.noNA.cast$steps)` in the dataset and corresponds to label interval `r activity.noNA.cast$interval[104]`. This interval contains `r round(activity.noNA.cast$steps[104],0)` steps on average across all the days in the dataset.
...
```{r, echo=TRUE, results="hide", message=FALSE} 
# strategy is to use the library MICE for multiple imputations
